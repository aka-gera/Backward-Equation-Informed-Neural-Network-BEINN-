...
from beinn.neld_fun_0.get_path import get_param,get_name
from beinn.neld_fun_0.help_funn import get_color,format_array,loadtxt,loadtxt_count
import pickle
...
class iou_train(get_name,get_param):
    def __init__(self, 
        ...
        # count= loadtxt( os.path.join(path_appr,self.txt_smod_count),dtype=int) 
        # count=format_array(count)
        
        count= loadtxt_count(os.path.join(path_appr,self.txt_smod_count))
        mmm=count.ndim
        ...
        # self.count_appr=count_appr=np.loadtxt(os.path.join(path_appr,self.txt_smod_count),dtype=int)


        self.hmod_path_appr=path_appr
        self.count_appr_tmp=self.count_appr 
        # print(self.count_appr_tmp,os.path.join(hmod_path_appr,self.txt_smod_count))
        # for ii in self.count_appr_tmp:
            # print(os.path.join(hmod_path_appr, f'{self.name_smod_index}_{ii}.txt'))
        if len(self.count_appr_tmp)>1:

            self.appr_index_data = {
                ii: set(np.loadtxt(os.path.join(path_appr, f'{self.name_smod_index}_{val}.txt'), dtype=int))
                for ii,val in self.count_appr_tmp.items() if os.path.exists(os.path.join(path_appr, f'{self.name_smod_index}_{val}.txt'))
            } 
        self.count_true= loadtxt(os.path.join(path_true,self.txt_smod_count),dtype=int) 
        if  self.count_true.ndim>0: 
            self.true_index_data = {
                jj: set(np.loadtxt(os.path.join(path_true, f'{self.name_smod_index}_{jj}.txt'), dtype=int))
    ...
    def get_mapping(self,save=None,iou_thre=None):
        ...
        if mp is not None:
            if save:
                with open(os.path.join(self.hmod_path_appr,self.pkl_mp), "wb") as file:
                    pickle.dump(mp, file)
        else:
            mp = {
                ii: {
    ...
    def get_iou_save(self,save=None,iou_thre=.7):
        ...
        if save:
            np.savetxt(os.path.join(self.hmod_path_appr,self.txt_smod_iou),np.array(save_iou), fmt="%4f")
            ...
            # with open(os.path.join( self.hmod_path_appr,'iou.csv'), 'w') as f: 
            #     f.write(',' + ','.join(m_name) + '\n') 
    ...
    def get_iou_match(self,save=None,iou_thre=.7,dice_thre=.7):
        if (self.file_path is not None)  :
            ...
            np.savetxt(os.path.join(self.hmod_path_appr,f'smod_match.txt'),intensity, fmt="%d")
            np.savetxt(os.path.join(self.hmod_path_appr,f'smod_match_dice.txt'),intensity_dice, fmt="%d")
            np.savetxt(os.path.join(self.hmod_path_appr,f'smod_annot.txt'),intensity_true, fmt="%d")
