...
from neld_fun_0.help_funn import get_intensity ,mappings_vertices,pca_projector
...
class model_manipulate(get_files,get_name):
    ...
    def get_wrap(self,   
        ...
        from neld_fun_0.get_wrap import get_wrap ,get_wrap_o3d
    ...
    def get_skeleton(self,   
        ...
        param_dic =param_dic if param_dic is not None else self.param_dic
        from neld_fun_2.help_pinn_data_fun import def_mesh_to_skeleton_finder
        from neld_fun_0.help_funn import order_points_along_pca,project_vertices_onto_skeleton
        ...
        for  index,neld_name in enumerate(neld_names):
            ...
            for key in self.neld_path_original_mm['dir']:  
                pid=pinn_data(file_path=self.file_path,
                                ...
                                hmod_path=     self.path_file[key] ,
                                smod_path=     self.path_file[key] , 
                                hmod_path_pre= self.path_file[key] ,
                                smod_path_pre= self.path_file[key] ,  
                                neld_path_original_m=self.neld_path_original_mm['dir'][key],
                                neld_first_name=self.neld_namess[index][1], 
                                name_smod_id=self.name_smod_id,
                                thre_target_number_of_triangles=thre_target_number_of_triangles,
                                voxel_resolution=voxel_resolution,
                ...
                pathi=os.path.join(self.obj_org_path_old,f'{self.neld_name}','data',f'{neld_name }.obj') 
                ...
                pa_list=[ff for ff in xxx
                        if ff.startswith(f'{self.neld_first_name}{self.name_smod_id}')
                        and not ff.endswith('hmod.obj')
                        ]    
                if not pa_list:
                    continue
                smod_path=fpath_true 
                ...
                for pa in pa_list:
                    ...
                    smod_index_all.extend(vertices_index) 
                    # np.savetxt(os.path.join(hmod_path_resized, f'{self.name_smod_index}_{ip}.txt'), nhn.vertices_index, fmt='%d') 
                    # np.savetxt(os.path.join(hmod_path_resized, f'{self.name_smod_faces}_{ip}.txt'), nhn.faces, fmt='%d') 
                    # np.savetxt(os.path.join(hmod_path_resized, f'{self.name_smod_index_unique}_{ip}.txt'), nhn.vertices_faces_unique, fmt='%d')

                    np.savetxt(os.path.join(smod_path, f'{self.name_smod_index}_{ip}.txt'), vertices_index, fmt='%d') 
                    np.savetxt(os.path.join(smod_path, f'{self.name_smod_faces}_{ip}.txt'), nhn.faces, fmt='%d')  
                    
                    count.append(ip)  



                np.savetxt(os.path.join(smod_path,'smod_count.txt'),np.array(count), fmt='%d')  
                np.savetxt(os.path.join(smod_path, self.txt_smod_intensity ), intensity_org, fmt='%d')
                ...
                if count:   
                    ...
                    np.savetxt(os.path.join(smod_path,'intensity_hmod_smod.txt'), intensity, fmt='%d')
                    np.savetxt(os.path.join(smod_path,'intensity_1hot_hmod_smod.txt'), intensity_1hot, fmt='%d')
            ...
            skl_name=os.path.join(neld_path_entry_data,f'{self.neld_first_name}_hmod_wrap.obj') 
            print('[[[[[[[[[[[[[]]]]]]]]]]]]]',skl_name)
            if os.path.exists(skl_name): 
                mesh=trimesh.load_mesh(skl_name, process=False) 
                mesh.export(os.path.join(file_path_feat_exit,f'{self.neld_first_name}_hmod_wrap.obj') ) 
            ...
            for pa in xxx:
                ...
                txt_distance=self.txt_skl_hmod_distance if wrap_part in pa else self.txt_skl_distance
                txt_vertices=self.txt_skl_hmod_vertices if wrap_part in pa else self.txt_skl_vertices 
    ...
    def get_smooth(self,   
        ...
        param_dic = param_dic if param_dic is not None else self.param_dic
        import neld_fun_0.curvature as cuHP 
        from neld_fun_2.help_pinn_data_fun import smooth_curvature
        ...
        for  index,neld_name in enumerate(neld_names):
            ...
            kname=[f for f in os.listdir(neld_path_entry_data) if  f.endswith('.obj') and f.startswith((f'{self.neld_first_name}{self.name_smod_id}',f'{self.neld_first_name}_hmod.obj'))]
    ...
    def get_resize(self,   
        ...
        from neld_fun_2.help_pinn_data_fun import mesh_resize
        ...
        for  index,neld_name in enumerate(neld_names):
            ...
            for entry_name,exit_name in zip(entry_names,exit_names):
                ...
                kname=[f for f in xxx if  f.endswith('.obj') and 
                    f.startswith((f'{self.neld_first_name}{self.name_smod_id}',f'{self.neld_first_name}_hmod.obj'))]
                kname.append(f'{neld_name }.obj')
                for pa in kname:  
    ...
    def get_diane_smods(self, 
                    entry_name_hmod=None,   
                    exit_name='wrap',
                    data_studied=None, 
                    disp_infos=None, 
                    neld_names=None,
        ...
        for  index,neld_name in enumerate(neld_names):
            ...
            if entry_name_hmod is None:
                neld_path_entry=os.path.join(f'{self.obj_org_path}',self.neld_name,'data_org',)
            else:
                neld_path_entry=os.path.join(f'{self.obj_org_path}_{entry_name_hmod}',self.neld_name,'data_org',)
            if exit_name is None:
                neld_path_exit=os.path.join(f'{self.obj_org_path}',self.neld_name,'data_org',)
            else:
                neld_path_exit=os.path.join(f'{self.obj_org_path}_{exit_name}',self.neld_name,'data_org',)
            ...
            hmod_path_file=neld_path_entry#os.path.join(self.obj_org_path,self.neld_name,'data_org')  
            mesh_org_path_file=smods_true_path=neld_path_exit#os.path.join(f'{self.obj_org_path}_{exit_name}',self.neld_name,'data_org',)
            os.makedirs(smods_true_path,exist_ok=True)  
            pid.get_diane_smods(neld_name,neld_first_name,name_smod_id,mesh_org_path_file,hmod_path_file,smods_true_path,k_query=k_query,size_threshold=size_threshold)
    ...
    def get_skl_hmod_pred(self, 
        ...
        print(f'get_skl_hmod_pred started')
        print('--------------------------------')
        ...
        for  index,neld_name in enumerate(neld_names):
            ...
            pid.get_neld_data()
            con=0
            path_hmod_dir=path_hmod_dir or f'{self.model_type}_{self.model_sufix}_{self.path_dir}'
            hmod_path=  self.path_file[path_hmod_dir]  
            # hmod_path= self.path_file[f'{self.model_type}_{self.model_sufix}_{self.path_dir}']
            hmod_path=  self.path_file[path_train['data_hmod_path']]  
            dest_path = self.path_file[path_train['dest_hmod_path']] 
            # print('[[[[[get_skl_hmod_pred location]]]]]',hmod_path)

            pid.get_skl_hmod_pred( 
                            hmod_path=hmod_path,
                            dest_path=dest_path,
                            weight=weight,
                            weights=weights, 
    ...
    def get_hmod_process(self, 
        ...
        print(f'get_hmod_process started')
        print('--------------------------------')
        print(f"Model Type Gen : {model_type}")
        print(f"Model Type     : {model_sufix}")
        print(f"Model Dir.     : {model_dir}")
        ...
        for  index,neld_name in enumerate(neld_names):
            ...
            smods_logit=self.intensity_logit_dict[pre_portion]        #head_neck_logit if pre_portion =='head_neck' else self.intensity_smods_logit
            for kk,tyy in enumerate(smods_logit): 
