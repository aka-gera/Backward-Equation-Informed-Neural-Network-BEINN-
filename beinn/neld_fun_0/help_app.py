...
class DSAPage:
    ...
    def __init__(self,path_heads_show=None,categories=None,path_display=None,dnn_modes=None):
        self.name = "dnn"
        self.page_path = "/DSA-2"
        self.categories = ["dsa", "dnn"]  if categories is None else categories
        self.path_display = ["dest_hmod_path"] if path_display is None else path_display
    ...
    def upload(self, args):
        ...
        *other_values, contents, filenames, last_modified, objs_path_org = values
        ...
        print('[[[[[[[[[===========]]]]]]]]]',param["path_dirs_weig"]["param"])
'''
        ...
        if isinstance(contents, list):
            ...
            for content, filename in zip(contents, filenames):
                ...
                neld_path = os.path.join(objs_path_org, filename, "data")
                os.makedirs(neld_path, exist_ok=True)
                parse_obj_upload(content, filename, export_dir=neld_path)
                neld_names.append(filename)
    ...
    def run_algorithm(self, store_data):
        ...
        '''
        nam=model_type.split('___')[1]
        nam_gen=nam.split('_')[:-1].join('_')'''
        print('[[[[[[model  ]]]]]]',model_type)
        nam = model_type.split('___')[1]
        nam_gen = nam.rsplit('_', 1)[0]  

        action='train'
        nam_g = model_type.rsplit('_', 1)[0]  
        model_typei=f'{nam_g}_{action}'


        path_heads_show=params['path_heads_show']
        neld_data = paraws[nam_gen]['gdas'].part(nam)
        path_dict = paraws[nam_gen]['path_dict']
        dict_param = paraws[nam_gen]['dict_param']
        # neld_data = gdas.part(nam)
        print('[[[[neld data]]]]',nam,nam_gen,nam_g)
