...
class dnn_PINN(keras.Model):
    def __init__(self,Dtype=DTYPE,hidden_layers = 4, neurons_per_layer = 150,
                 n_classes=1,
                 activation_init="tanh",
        ...
        self.activation_last=activation_last
        k_regularizers=regularizers.l1_l2(l1=l1,l2=l2)
        super(dnn_PINN, self).__init__()
        self.ld_init = tf.keras.layers.Dense(neurons_per_layer, activation="tanh", dtype =Dtype )
        self.ld = tf.keras.layers.Dense(neurons_per_layer, 
                  kernel_regularizer=k_regularizers,
                  activation="tanh",dtype = Dtype)
        self.ld_last = tf.keras.layers.Dense(n_classes, dtype = Dtype)
        ...
        **kwargs,
        ...
        for key, entry in self.models.items():
            if model_type.startswith(key):
                ...
                # Models that use filters
                if cls in (dnn_PointNet, dnn_1GINN_SM,dnn_PINN):
                    filters = kwargs.pop("filters", entry["filters"])
                    return cls(
                        filters=filters,
                        n_classes=n_classes,
                ...
                # Models that don't use filters
                elif cls is dnn_GINN:
                    return cls(
                        n_classes=n_classes,
                        activation_init=self.activation_init,
                        activation_hidden=self.activation_hidden,
...
def Get_iou(model, curv, lab,adj=None,get_model_one_hot=None,neld=None):
    iou_s = {0: [], 1: [], 2: []}
...
def get_auc(model, curv, rhs,adj=None,get_model_one_hot=None,neld=None):
    auc_s = {0: [], 1: [], 2: []}
    y_true,score=[],[]
    ...
    if score and y_true:
        ...
        for label,(yy,sc,nm) in enumerate(zip(yy_true.T,y_score.T,['hmod','smod'])):
            # print('=============',yy.shape)
