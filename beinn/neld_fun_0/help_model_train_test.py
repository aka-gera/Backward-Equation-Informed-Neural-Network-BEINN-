...
class train_test_tf(get_files,get_name): 
    ...
    def train_model_smod_cml(self,
        ...
        curv,rhs,adj,neld,weights =self.get_train_input_cml(   
                                path_train=path_train,  
    ...
    def get_train_input_gcn(self,  
        ...
        for entry_name in entry_names:
            for neld_name in neld_names: 
                ...
                adj.append(hff.adjoint(vertices=pid.neld.vertices,faces=pid.neld.faces,num_sub_nodes=num_sub_nodes))
                # curv.append(np.hstack(pid.get_pinn_features(feat_paths=feat_paths,base_features_list=base_features_list,))) 
                adjs=hff.adjoint(vertices=pid.neld.vertices,faces=pid.neld.faces,num_sub_nodes=num_sub_nodes) 
                feat=pid.neld.vertices[adjs[1]]
                if len(list(list_features)+list(base_features_list))>0:
                ...
                neld.append(pid.neld)   
                # curv.append(get_inputs(X=pid.neld.vertices[adjs[1]] ,adj=adjs[0]))
                if tf_train: 
                    rhs.append(tf.cast(
                        pid.get_pinn_rhs(pre_portion=pre_portion,
                                        file_path_feat=file_path_feat,
    ...
    def train_model_smod_gcn(self,
        ...
        curv,rhs,adj,neld,weights =self.get_train_input_gcn(   
                                path_train=path_train,  
                                pre_portion=pre_portion, 
                                line_num_points_hmod=line_num_points_hmod,
                                line_num_points_inter_hmod=line_num_points_inter_hmod, 
        ...
        if new_model:
            ...
            print(f"neld Trainer size    : {[(fv['x'].shape[0],fv['x'].shape[1]) for fv in curv_train]}")
    ...
    def get_train_input_volume(self,  
        ...
        for entry_name in entry_names:
            for neld_name in neld_names: 
                ...
                vol=mskl.get_volume() 
                if not tf_train: 
                    vol,idx_new=mskl.get_volume_crop(mesh_hmod_wrap=None,fill_method="orthographic")
                ...
                mesh_hmod_wrap = trimesh.load_mesh(os.path.join(file_path_feat, f'{self.neld_first_name}_hmod_wrap.obj'))

                vol,mask,idx_org =mskl.get_volume_crop(mesh_hmod_wrap=mesh_hmod_wrap,fill_method="orthographic") 
        ...
        return volumes, masks,mskls,smod_indexs,idx_originals,weight,rhs
 

    def train_model_smod_cnn(self,
                            path_train=None,
                            get_training=True, 
                            pre_portion=None,
                            full_neld=True, 
        ...
        curv,rhs,mskls,smod_indexs,idx_originals,weight_mask,rhshr =self.get_train_input_volume(   
                                path_train=path_train,  
                                pre_portion=pre_portion, 
                                line_num_points_hmod=line_num_points_hmod,
                                line_num_points_inter_hmod=line_num_points_inter_hmod, 
        ...
        if new_model:
            ...
            print(f"Target Size    : {rhs[0].shape[1]} * {len(rhs)}")
            print(f"neld Trainer size    : {[tuple(fv.shape) for fv in curv_train]}")
            print(f"Hidden Layers        : {hidden_layers}")
            print(f"neurons_per_layer    : {neurons_per_layer}")
            print('--------------------------------')
            loss_save = []
    ...
    def train_model_smod_dnn(self,
        ...
        curv,rhs,adj,neld,weights =self.get_train_input_dnn(   
                                path_train=path_train,  
                                pre_portion=pre_portion, 
                                line_num_points_hmod=line_num_points_hmod,
                                line_num_points_inter_hmod=line_num_points_inter_hmod, 
    ...
    def get_hmod_pred_dnn(self, 
        ...
        for  index,neld_name in enumerate(neld_names):
            ...
            smods_logit=  self.intensity_logit_dict[pre_portion]
            key,tyy=f'{self.model_type}_{self.model_sufix}_{self.path_dir}', smods_logit[0]  
            path_ex = os.path.join(self.path_file_sub[tyy][key]) 
            if os.path.exists(path_ex) and not param_dic['tf_restart']['get_hmod_pred']: 
                continue
            pid.save_pinn_data() 
            ...
            smods_logit=self.intensity_logit_dict[pre_portion]
            print('[[[[[[[[[[[[[[[[[intensity_smods_logit]]]]]]]]]]]]]]]]]',smods_logit)
            for kk,tyy in enumerate(self.intensity_logit_dict[pre_portion]):
                np.savetxt(self.path_file_sub[tyy][key], rhs0[:,kk][idx], fmt='%f') 
    ...
    def train_model_smod_PINN(self,
                            path_train=None,
                            get_training=True, 
                            pre_portion=None,
                            full_neld=True, 
        ...
        curv,rhs,adj,neld =self.get_train_input_PINN(   
                                path_train=path_train,  
                                pre_portion=pre_portion, 
                                line_num_points_hmod=line_num_points_hmod,
                                line_num_points_inter_hmod=line_num_points_inter_hmod, 
    ...
    def train_model_smod(self,
        ...
        curv,rhs,adj,neld =self.get_train_input(   
                                path_train=path_train,  
                                pre_portion=pre_portion, 
                                line_num_points_hmod=line_num_points_hmod,
                                line_num_points_inter_hmod=line_num_points_inter_hmod, 
        ...
        for  index,neld_name in enumerate(neld_names):
            ...
            if model_type.startswith(('cML', 'ML')): 
                ...
                if model_type.startswith(('pinn', 'rpinn')):
                    ...
                    adj=hff.adjoint(vertices=pid.neld.vertices,faces=pid.neld.faces,num_sub_nodes=None)
