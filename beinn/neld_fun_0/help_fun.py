...
class geometry:
    def __init__(self,vertices,index=None,line_num_points = 1000) -> None: 
        ...
        neld_points_start = centroid +  proj_0.min()* princi_dir
        neld_points_end= centroid +  proj_0.max()* princi_dir 
        self.neld_points_start,self.neld_points_end=neld_points_start,neld_points_end
    ...
    def Cluster_PCA(self,cluster_index=None):
        ...
        cluster_pca_points_proj=Closest_point_on_line(self.neld_points_start, self.neld_points_end, point=cluster_pca_points)
    ...
    def Lines(self):
        tt=self.tt
        self.pca_line = np.outer(1-tt, self.neld_points_start) + np.outer(tt,self.neld_points_end)
...
class geometry:
    ...
    def Cluster_index_filter(self,cluster_index):
        ...
        vector_director_2=self.cluster_centroid-self.cluster_farthest_point
        cluster_points_proj=Closest_point_on_line(self.neld_points_start, 
                                                         self.neld_points_end, 
...
def create_scatter_plot(cluu,neld, geo_cluster ): 
    geo_cluster.Cluster_PCA(cluster_index=cluu)
    geo_cluster.Lines()

    intensity = neld.mean_curv()[cluu]
    vertices_ = neld.vertices[cluu]
...
def create_subplots(clu, clu_new ,neld,geo_cluster , width=500, height=400):
    # Create subplots
    ...
    scatter_1 = create_scatter_plot(clu, neld,geo_cluster )
    for scatter in scatter_1:
        fig.add_trace(scatter, row=1, col=1)
 
    scatter_2 = create_scatter_plot(clu_new, neld,geo_cluster )
...
# class neck:
#     def __init__(self,vertices_0,node:hff.model) -> None: 
