...
def get_neld_data(self,flow, epsilon, nPart, rcut, N, Nperiod,force,beta,gamma,
    ...
    for  index,neld_name in enumerate(self.neld_names):
        ...
        if not get_new_data:
            ...
            sav_part =sav_all= saves(pm)

            sav_all,XX=Fun().Simulation_init(pm, X_init,sav_part, dt_r=dt_r1,Fun_integ=Fun_integ)
            '''        
            Tinit=dict(count=0,
                    range_0=0,
                    range_N=1)
        ...
        for ii in range(Nmarkov):
            ...
            if tf_time:
                path_time=os.path.join(path_1,f"time.txt")
                np.savetxt(path_time,sav.time[:Ntime,:],fmt=fmt)
                tf_time=False
            path_qq=os.path.join(path_1,f"qq_{ii}.txt")
            path_pp=os.path.join(path_1,f"pp_{ii}.txt")
            path_ff=os.path.join(path_1,f"ff_{ii}.txt")
            np.savetxt(path_qq,sav.qq,fmt=fmt)
            np.savetxt(path_pp,sav.pp,fmt=fmt)
            np.savetxt(path_ff,sav.fDist,fmt=fmt)
...
def lhs_g(time=None,qq=None,pp=None,qq2=None,pp2=None,ff=None,name='pp',vol=None,gg=None): 
    base=[time,qq,pp,gg]
    if name in (['pp','qq']):
        pass  
    elif name=='pressure':
...
def get_neld_data_train(self,
    ...
    Xall=dict(zip(['time','q','p','force','g'],[{nn:[] for nn in keyss} for _ in range(5)]))
    ...
    timm=None
    # Nmarkov=1000
    Nsample=Nsample if Nsample is not None else len(self.neld_names) 
    ...
    for key,val in zip(keyss,Samples_nbr):
        ...
        for index in val:
            ...
            if timm is None:
                timm = np.tile(time, (1, (pm.Nperiod))).T
            ...
            print('[[[[[[]]]]]]',pressure.shape,timm.shape,pp.shape)
            ...
            co = lhs_g(name=g_name,pp=pp_0,qq=qq_0,qq2=qq_0**2,pp2=pp_0**2,ff=ff_0,vol=pm.vol,time=time_0,gg=pressure_0)
            ...
            # co=np.concatenate([np.vstack([val for val in vals]) for vals in [time_0,qq_0,pp_0]], axis=1)
            press=np.zeros((pm.N* (pm.Nperiod),pm.dimm*pm.nPart))
            for ii in range(Nmarkov):
                path_qq=os.path.join(path_1,f"qq_{ii}.txt")
                path_pp=os.path.join(path_1,f"pp_{ii}.txt")
                path_ff=os.path.join(path_1,f"ff_{ii}.txt")
                ...
                print('[[[[MM[[]]]]]]',pressure.shape,timm.shape,ppt.shape,index,ii,path_qq)
                # pressure = qq
                # pressure = np.sum(qq**2 , axis=1).reshape(-1,1)
                if ii>Nmarkov-10:
            ...
            cood = lhs_g(name=g_name,pp=pp_f,qq=qq_f,pp2=pp_f2,qq2=qq_f2,ff=ff_f,vol=pm.vol,time=time_f,gg=u_f) 
            cood=np.concatenate([np.vstack([val for val in vals]) for vals in cood], axis=1)
            xPress=press.reshape((pm.Nperiod), pm.N, pm.dimm*pm.nPart).transpose(1, 0, 2).reshape(pm.N, (pm.Nperiod)*pm.dimm*pm.nPart)
...
class aka_grad():
    ...
    def grad_qp(self,model,fun_coef):  
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            tape.watch(q)
            tape.watch(p)
            coord = lhs_g(name=g_name,pp=p,qq=q,qq2=q**2,pp2=p**2,ff=f,time=t,gg=g)
...
class lang_loss():
    ...
    def call(self,model,dc):  
        ...
        u,u_t,u_q,u_p,u_pp = aka_grad().grad_qp(model,dc) 
        '''
        if dc.flow == 'shear':
...
'''
        ...
        for val in dc.bcs.values():
            if val['tf']:  
                loss+=  aka_grad().loss_fn(model(val['coord']),val['u'])
...
class neld_coef(PBCs):
    def __init__(self,par,
        ...
        if par is not None:
            ...
            self.g = tf.cast(par['Xall']['g']['train'],dtype=DTYPE)


            # Gamma=np.zeros_like(sav.qq[Nterm,:])
            Gamma=np.zeros_like(par['Xall']['q']['train'])
            A=self.A+self.gamma*np.eye(self.dimm)
