...
# import torch
import dend_fun_0.help_neld_train_test as hntt
import random
np.random.seed(42)
        ...
        for head_neck_path in self.path_display:
            ...
            for  index,dend_name in enumerate(dend_names):
                ...
                for true_path,true_key in self.dend_path_original_mm['keys'].items() : 
                    ...
                    path_grap_true=os.path.join(dend_path_true_final ,'intensity_1hot_hmod_smod.txt') 
        ...
        for  index,dend_name in enumerate(dend_names):
            ...
            pid=pinn_data(file_path= file_path,
                          file_path_feat= file_path_feat,
                            hmod_path=hmod_path,
                            smod_path=smod_path,
                            hmod_path_pre=self.path_file[path_train['dest_hmod_path']],
                            dend_path_original_m=self.dend_path_original_m, 
                            dend_first_name=self.dend_namess[index][1],  
                            path_train=path_train, 
                            thre_target_number_of_triangles=thre_target_number_of_triangles,
            ...
            for center_path in self.path_display:
                smod_path=self.path_file[self.path_train[center_path]]
                hmod_vertices_center_path=self.path_file[self.path_train['dest_hmod_path']] 
                if len(self.obj_org_path_dict)>0:
                    for ii,(keys,val) in enumerate(self.obj_org_path_dict.items()): 
                        ...
                        count_path=os.path.join(smod_path, self.txt_hmod_vcv_length )
                        if os.path.exists(count_path) :
                            pid.get_dend_data()  
                            # pid.get_central_data()  
                            # giou=graph_iou( pid,smod_path,true_path,hmod_vertices_center_path) 
                            # giou.scatter_center()  
                            vertices_00= np.loadtxt(os.path.join(file_path, self.txt_vertices_old), dtype=float)
                            scatter_smod=get_scatter_center(self, smod_path,hmod_vertices_center_path,vertices_00) 
                            with open(os.path.join(smod_path,'plot_data_center_curv.pkl'), 'wb') as f:
                                pickle.dump(scatter_smod, f)  
    ...
    def get_cylinder_heatmap(self,   
        ...
        for  index,dend_name in enumerate(dend_names):
            ...
            for center_path in self.path_display: 
                ...
                count_path=os.path.join(smod_path,self.txt_hmod_vcv_length )
                if os.path.exists(count_path):
                    gcyl=graph_cylinder_heatmap( pid, smod_path=smod_path)
                    gcyl.get_cylinder_heatmap() 
                else:
                    print('[[[[[[[[[[get_cylinder_heatmap  Fail ]]]]]]]]]]',center_path,smod_path)
