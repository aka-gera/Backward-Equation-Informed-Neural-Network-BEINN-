...
from dend_fun_0.help_model_pred import model_pred 
from dend_fun_0.get_path import get_path_train,get_data_mode  
...
def get_dict_param(nam='meshes',
    ...
    # dnn_modes=dnn_modes if dnn_modes is not None else ['DNN-0','DNN-1','DNN-2' , 'DNN-3',]
    ...
    param_dic={
        hh:{
            ...
            'get_smooth','get_hmod_pred','get_dend_name','get_skeleton', ]
        }
...
def algorithm_param(nam='meshes',
    ...
    param_input=['model-pred', 'clean_path_dir', ]
    param_inputbv=[ 'intensity_rhs','intensity_all','iou','roc','graph_center','dash_pages', 
                   'cylinder_heatmap' ,'annotations', 'model_pred' , 'rhs','skl','Skeleton','Smooth','Resizing',  'Morphologic Param','model_shap', ]
    param_input_save=[ 'smods_segss_old','smods_segss_old_2', 'clean_path_dir','get_training','skl_hmod_pred']
    param_list=['param_dropdown','param_input','param_all', 'path_dirs_weig', 'param_list','param_dropdowni','param_inputbv','path_list','head_navbar','path_heads_show_nami',]
    param_all=[]
...
class app_run_param:
    def __init__(self,param):
        ...
        for gval,gvali,mb in zip(param['param_dropdown']['param'],param['param_dropdowni']['param'],[0,-1,1,-1]):
                            ...
                            [
                                dbc.Row([
                                    dbc.Card(
                                        dbc.CardBody([
                                            html.Div(
                                                dcc.Markdown('# Backward Equation-Informed Neural Network (BEINN)',
                                ...
                                dbc.Row([
                                    dbc.Col(
                                        dcc.Markdown(
                                            "[View the full repository on GitHub](https://github.com/aka-gera/CURVATURE-BASED-MACHINE-LEARNING-FOR-AUTOMATED-SEGMENTATION-OF-DENDRITIC-smodS)",
                                            style={"textAlign": "center", "color": "white"}
                                        )
                                    )
                                ], justify="center")
 ...
 '''
    ...
    def test( self,   
        ...
        for ixi,mode_id in enumerate(mode_ids):#data_mode.keys(): # 
            ...
            if param['rhs']['tf']: 
                dend_cla.get_rhs(
                                        param_dend_name=param['model_pred']['param']['param_dic']['data']['get_dend_name'],
                                        **param['Skeleton']['param'])
         

            if param['model-pred']['tf']:   
                # if (dnn_mode== 'DNN-2'):
                #         dend_cla.get_skl_hmod_pred(
                #                                 # weights=weights,
                #                                 path_hmod_dir=path_hmod_dir,
                #                                 train_smods=train_smods,
                #                                 model_type=model_type,
                ...
                if model_type.startswith(('vol','cnn')):
                    ...
                    dend_cla.get_hmod_pred(
                                            # weights=weights,
                                            train_smods=train_smods,
                                            model_type=model_type,
                                            **param['model-pred']['param']
                                        )    
                # dend_cla.get_hmod_process(
                #                         # weights=weights,
                #                         train_smods=train_smods,
                #                         model_type=model_type,
                #                         **param['model-pred']['param']
        ...
        for ixi,mode_id in enumerate(mode_ids):#data_mode.keys(): #
            ...
            if param['get_training']['tf']:
                if model_type.startswith(('cml','ML','cML')): 
                    ...
                    dend_cla.train_model_smod_PINN(
                                            train_smods=train_smods, 
                                            model_type=model_type, 
                                            model_sufix=model_sufix, 
                                            entry_names=entry_names,
                    ...
                    dend_cla.train_model_smod_pnet(
                                            train_smods=train_smods, 
                                            model_type=model_type, 
                                            model_sufix=model_sufix, 
                                            entry_names=entry_names,
        ...
        for ixi,mode_id in enumerate(mode_ids):#data_mode.keys(): # 
            ...
            for nn in ['entry','exit']:
                if len(save_entry_exit[nn])==0:
                    ...
                    param['model_pred']['param']['param_dic']['data']['get_dend_name']['old_path']='current'
                save_entry_exit['old_path'].append('entry')

 
            if param['Resizing']['tf']: 
                # param['model_pred']['param']['param_dic']['data']['get_dend_name']['dict_dend_path']=dict(dict_dend_path='old',
                #                                 drop_dic_name=save_entry_exit['exit'][-1])
                param['model_pred']['param']['param_dic']['data']['get_dend_name']['dict_dend_path']= 'old'
                param['model_pred']['param']['param_dic']['data']['get_dend_name']['drop_dic_name']= save_entry_exit['exit'][-1]
                param['model_pred']['param']['param_dic']['data']['get_dend_name']['old_path']='current'
            ...
            if resize is not None:
                param['model_pred']['param']['param_dic']['data']['get_dend_name']=dict(dict_dend_path='old',
                                                                    old_path=None,
            ...
            for entry_name,exit_name,old_path in zip(save_entry_exit['entry'],save_entry_exit['exit'],save_entry_exit['old_path']):
                for paath in ['entry','old']:
                    ...
                    if param['Skeleton']['tf']: 
                        ...
                        # print('[[[[[[[--------------skl------------]]]]]]]',param['model_pred']['param']['param_dic']['data']['get_dend_name'] )
                        param_dend_name=deepcopy(param['model_pred']['param']['param_dic']['data']['get_dend_name'])
                        ...
                        dend_cla.get_wrap(wrap_part='hmod', 
                                        alpha_fraction=1,
                                        offset_fraction=.7,
                                        entry_name=entry_name,
                                        exit_name=exit_name,
...
class get_data_all:
    def __init__(self, dend_data=None,names_dic={},obj_org_path=None,file_path_data=None,true_keys=[f'true_0',],cpath=None, ):  
        self.dend_data=dend_data   
