        ...
        for entry_name in entry_names:
            for dend_name in dend_names: 
                ...
                if tf_train:
                    ...
                    if pre_portion=='head': 
                        pathh=os.path.join( self.dend_path_true_final ,'intensity_hmod.neck_head.txt')
                    elif pre_portion=='neck_head': 
                        pathh=os.path.join( self.dend_path_true_final ,'intensity_hmod_neck_head.txt')
                    else:
                        pathh=os.path.join( self.dend_path_true_final ,'intensity_hmod_smod.txt')
    ...
    def get_train_input_volume(self,  
        ...
        for entry_name in entry_names:
            for dend_name in dend_names: 
                ...
                mesh_hmod_wrap = trimesh.load_mesh(os.path.join(file_path_feat, f'{self.dend_first_name}_hmod_wrap.obj'))
        ...
        for entry_name in entry_names:
            for dend_name in dend_names: 
                ...
                if tf_train:
                    ...
                    #         ) 
                    rhs.append(tf.cast(
                        np.loadtxt(os.path.join( self.dend_path_true_final ,f'intensity_1hot_hmod_{pre_portion}.txt'),dtype=int), 
                        dtype=DTYPE)
                            ) 
                    pathh=os.path.join( self.dend_path_true_final ,f'intensity_hmod_{pre_portion}.txt')
                    mask=np.loadtxt(pathh,dtype=int)
                    unique=np.sort(np.unique(mask))
                    labels = {v: np.argwhere(mask == v) for v in unique}
                    counts = {v: labels[v].shape[0] for v in labels}
