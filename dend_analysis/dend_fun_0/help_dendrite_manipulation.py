...
class model_manipulate(get_files,get_name):
        ...
        for  index,dend_name in enumerate(dend_names):
            ...
            for key in self.dend_path_original_mm['dir']:  
                ...
                pa_list=[ff for ff in xxx
                        if ff.startswith(f'{self.dend_first_name}{self.name_smod_id}')
            ...
            skl_name=os.path.join(dend_path_entry_data,f'{self.dend_first_name}_hmod_wrap.obj') 
            print('[[[[[[[[[[[[[]]]]]]]]]]]]]',skl_name)
            if os.path.exists(skl_name): 
                mesh=trimesh.load_mesh(skl_name, process=False) 
                mesh.export(os.path.join(file_path_feat_exit,f'{self.dend_first_name}_hmod_wrap.obj') ) 
    ...
    def get_smooth(self,   
        ...
        for  index,dend_name in enumerate(dend_names):
            ...
            kname=[f for f in os.listdir(dend_path_entry_data) if  f.endswith('.obj') and f.startswith((f'{self.dend_first_name}{self.name_smod_id}',f'{self.dend_first_name}_hmod.obj'))]
        ...
        for  index,dend_name in enumerate(dend_names):
            ...
            for entry_name,exit_name in zip(entry_names,exit_names):
                ...
                kname=[f for f in xxx if  f.endswith('.obj') and 
                    f.startswith((f'{self.dend_first_name}{self.name_smod_id}',f'{self.dend_first_name}_hmod.obj'))]
                kname.append(f'{dend_name }.obj')
                for pa in kname:  
    ...
    def get_diane_smods(self, 
        ...
        for  index,dend_name in enumerate(dend_names):
            ...
            if entry_name_hmod is None:
                ...
                dend_path_entry=os.path.join(f'{self.obj_org_path}_{entry_name_hmod}',self.dend_name,'data_org',)
            if exit_name is None:
                dend_path_exit=os.path.join(f'{self.obj_org_path}',self.dend_name,'data_org',)
            else:
                dend_path_exit=os.path.join(f'{self.obj_org_path}_{exit_name}',self.dend_name,'data_org',)
            ...
            hmod_path_file=dend_path_entry#os.path.join(self.obj_org_path,self.dend_name,'data_org')  
            mesh_org_path_file=smods_true_path=dend_path_exit#os.path.join(f'{self.obj_org_path}_{exit_name}',self.dend_name,'data_org',)
            os.makedirs(smods_true_path,exist_ok=True)  
            pid.get_diane_smods(dend_name,dend_first_name,name_smod_id,mesh_org_path_file,hmod_path_file,smods_true_path,k_query=k_query,size_threshold=size_threshold)
    ...
    def get_skl_hmod_pred(self, 
        ...
        for  index,dend_name in enumerate(dend_names):
            ...
            minutes, seconds = divmod(rem, 60)
            if disp_infos:
                print(f'hmod Prediction completed on {dend_name} in {int(hours)}h {int(minutes)}m {seconds:.2f}s')
